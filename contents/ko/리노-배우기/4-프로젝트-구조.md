...
- `rino-path`: 컴포넌트를 불러오기 위해 필요한 속성입니다. 예: `rino-path="/header"` 또는 `rino-path="/ko/footer"`

- `rino-tag`: 선택 속성입니다. 컴포넌트 태그에 HTML 속성을 사용하려면 HTML 태그 이름을 지정해야 합니다. 예: `rino-tag="div"` 또는
...
- `rino-type`: 마크다운을 사용하려면 이 속성에 `md` 또는 `markdown`을 지정해야 합니다.
- `rino-path`: 마크다운을 불러오기 위한 경로를 지정합니다.
- `rino-tag`: 선택 속성입니다. HTML 속성을 사용하려면 HTML 태그 이름을 지정해야 합니다.
